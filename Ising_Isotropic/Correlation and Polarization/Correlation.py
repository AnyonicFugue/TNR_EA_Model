...
@numba.jit(nopython=no_py, cache=cached)
def init(J, T, N0, beta,h,diag):  # N0 is the initial size of the TN
                ...
                for b in range(2):
                    ...
                    for c in range(2):
                        ...
                        for d in range(2):
                            ...
                            T[i][j][a][b][c][d] = -h*(2*(a+b+c+d)-4)  # Field; bring about effect of -hs
                            T[i][j][a][b][c][d] += (tmp[1]*tmp[3]+tmp[0]*tmp[2])*diag #Diagonal interaction
...
def main():
    ...
    corr_01 = np.zeros((kT_num, sample_num))
    corr_11 = np.zeros((kT_num, sample_num))
    ...
    #Now we only need correlation function and polarization. Heat capacity and Free energy are not necessary.
    ...
    for i in range(kT_num):  # Run over all temperatures
        ...
        for j in range(sample_num):
            ...
            #J0=2*np.random.randint(0,2,(2**n, 2**n, 4))-1 #\pm1 Coupling
            #J0 = np.random.randn(2**n, 2**n, 4)/2 #Gaussian Coupling

            Z[i][j], Q[i][j], F[i][j] = calculate(J0, 1/kT_array[i],n,h,0)
            z3,q3,f3=calculate(J0+step, 1/kT_array[i],n,h,0)
            z4,q4,f4=calculate(J0, 1/kT_array[i],n,h,step)

            while Q[i][j]<0 or q3<0 or q4<0:
                J0 = np.random.randn(2**n, 2**n, 4)/2 #Gaussian Coupling
            ...
            corr_01[i][j]=-(kT_array[i])*(q3-Q[i][j])/(step*(2**(2*n+1)))/2 #The pair number is twice the spin number, thus the extra 2.
            corr_11[i][j]=-(kT_array[i])*(q4-Q[i][j])/(step*(2**(2*n+1)))/2
    ...
    corr_01_average = np.zeros(kT_num)
    corr_11_average = np.zeros(kT_num)
    ...
    for i in range(kT_num):
        ...
        '''
        corr_01_average[i]=np.average(corr_01[i])
        corr_11_average[i]=np.average(corr_11[i])
    ...
    plt.plot(kT_array,corr_01_average)
    plt.show()
    plt.plot(kT_array,corr_11_average)
    plt.show()
    
    
    res = open("C.txt", mode='a')
    ...
    res.write(str(corr_01_average)+'\n')
    res.write(str(corr_11_average)+'\n')
    res.write('\n')
    res.close()
    
    res = open("C_full.txt", mode='a')
    res.write("Run time="+str(end-start)+'\n')
    res.write("n="+str(n)+",ncutoff="+str(ncutoff)+",kT_num=" +
              str(kT_num)+",sample_num="+str(sample_num)+'\n')
    res.write(str(kT_array)+'\n')
    ...
    #res.write(str(E_average)+'\n')
    #res.write(str(C_average)+'\n')
    res.write(str(corr_01)+'\n')
    res.write(str(corr_11)+'\n')
    res.write('\n')
    res.close()
