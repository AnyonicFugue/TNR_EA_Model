...
def main():
    ...
    for i in range(kT_num):  # Run over all temperatures
        ...
        for j in range(sample_num):
            ...
            #z2, q2, f2 = calculate(J0, 1/kT_array[i]+2*step,n,h)

            #E[i][j] = (q1-Q[i][j])/step
            #C[i][j] = ((q2-q1)-(q1-Q[i][j]))/(step*step)/((kT_array[i])**2)

            z1, q1, f1 = calculate(J0, 1/kT_array[i],n,h+step)


            z3,q3,f3=calculate(J0, 1/kT_array[i],n,h)
            ...
            Chi[i][j]=(kT_array[i])*((q5-q4)-(q4-q3))/((step*step))/(2**(2*n+1))/2 #Calculate susceptibility
