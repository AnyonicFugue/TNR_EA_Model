...
@numba.jit(nopython=no_py, cache=cached)
def init(J, T, N0, beta, h):  # N0 is the initial size of the TN
                ...
                for b in range(2):
                    ...
                    for c in range(2):
                        ...
                        for d in range(2):
                            ...
                            # Field; bring about effect of -hs
                            T[i][j][a][b][c][d] = -h*(2*(a+b+c+d)-4)
...
def main():
    n = 5  # Size of tensor network is 2**n * 2**n
    h = 0  # External field
    n_low=3
    n_high=9
    n_num=n_high-n_low
    ...
    F = np.zeros((n_num, sample_num))
    Z = np.zeros((n_num, sample_num))
    Q = np.zeros((n_num, sample_num))
    C = np.zeros((n_num, sample_num))
    E = np.zeros((n_num, sample_num))

    M = np.zeros((n_num, sample_num))
    Chi = np.zeros((n_num, sample_num))

    #kT_array = np.geomspace(low_kT, high_kT, kT_num)

    kT = 2.27
    n_array=np.array(range(n_low,n_high))
    #beta_array = kT_array
    #beta_array = np.linspace(0.05,10,kT_num)

    step = 0.0003

    for i in range(n_num):  # Different system sizes
        # Repeate several times for spin glasses. Useless for Ising.
        n = n_array[i]
        j = 0
        print("currently at "+str(n)+'\n')
        ...
        for j in range(sample_num):
            ...
            Z[i][j], Q[i][j], F[i][j] = calculate(J0, 1/kT, n, h)
            z1, q1, f1 = calculate(J0, 1/kT+step, n, h)
            z2, q2, f2 = calculate(J0, 1/kT+2*step, n, h)

            E[i][j] = (q1-Q[i][j])/step/(2**(2*n+1))
            C[i][j] = ((q2-q1)-(q1-Q[i][j]))/(step*step)/(2**(2*n+1))/(kT*kT)

            h = 0.008/kT

            z3, q3, f3 = calculate(J0, 1/kT, n, h)
            z4, q4, f4 = calculate(J0, 1/kT, n, h+step)
            z5, q5, f5 = calculate(J0, 1/kT, n, h+2*step)

            # The extra 2 is due to each spin is calculated twice
            M[i][j] = (kT)*(q1-Q[i][j])/(step)/(2**(2*n+1))/2
            Chi[i][j] = (kT)*((q5-q4)-(q4-q3))/((step*step)) / \
                (2**(2*n+1))/2  # Calculate susceptibility
    ...
    E_average = np.zeros(n_num)
    C_average = np.zeros(n_num)
    M_average = np.zeros(n_num)
    Chi_average = np.zeros(n_num)

    for i in range(n_num):
        E_average[i] = np.average(E[i])
        C_average[i] = np.average(C[i])

        M_average[i] = np.average(M[i])
        Chi_average[i] = np.average(Chi[i])
    ...
    plt.plot(n_array, E_average)
    plt.show()
    plt.plot(n_array, C_average)
    plt.show()
    plt.plot(n_array, M_average)
    plt.show()
    plt.plot(n_array, Chi_average)
    plt.show()

    res = open("result.txt", mode='a')
    res.write("Run time="+str(end-start)+'\n')
    res.write("n="+str(n)+",ncutoff="+str(ncutoff)+",n_num=" +
              str(n_num)+",sample_num="+str(sample_num)+'\n')
    res.write(str(n_array)+'\n')
    
    res.write(str(E_average)+'\n')
    res.write(str(C_average)+'\n')
    res.write(str(M_average)+'\n')
    res.write(str(Chi_average)+'\n')
