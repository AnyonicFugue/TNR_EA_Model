...
def main():
    ...
    M = np.zeros((kT_num, sample_num))
    Chi= np.zeros((kT_num, sample_num))
    ...
    for i in range(kT_num):  # Run over all temperatures
        ...
        for j in range(sample_num):
            ...
            Z[i][j], Q[i][j], F[i][j] = calculate(J0, 1/kT_array[i],n,h)
            z1, q1, f1 = calculate(J0, 1/kT_array[i],n,h+step)
            z5,q5,f5=calculate(J0, 1/kT_array[i],n,h+2*step)

            while Z[i][j]==-1 or z1==-1 or z5==-1:
                J0=2*np.random.randint(0,2,(2**n, 2**n, 4))-1 #\pm1 Coupling
            ...
            M[i][j]=(kT_array[i])*(q1-Q[i][j])/(step)/(2**(2*n+1))/2 #The extra 2 is due to each spin is calculated twice
            Chi[i][j]=(kT_array[i])*((q5-q1)-(q1-Q[i][j]))/((step*step))/(2**(2*n+1))/2 #Calculate susceptibility
    ...
    M_average = np.zeros(kT_num)
    Chi_average=np.zeros(kT_num)
    m_median=np.abs(np.median(M,axis=None))
    Chi_median=np.abs(np.median(Chi,axis=None))
    ...
    for i in range(kT_num):
        ...
        '''
        M_average[i]=np.median(M[i])
        Chi_average[i]=np.median(Chi[i])
    ...
    plt.plot(kT_array,M_average)
    plt.show()

    plt.plot(kT_array,Chi_average)
    plt.show()
    

    
    res = open("PS.txt", mode='a')
