...
def reduce(T, sx, sy, nx, ny):
    ...
    d_out = np.zeros((int(sy/2),ny),dtype=np.double) # return a 1-dim vector,dtype=np.double)
...
def calculate(J0, beta):
    ...
    if(np.isnan(res_q) or np.isinf(res_q)):
        print("nan!")
    res_f = -res_q/beta
...
def main():
    ...
    kT_array = np.linspace(5,10, kT_num)
    ...
    for i in range(kT_num):  # Run over all temperatures
        ...
        while(j<sample_num):
            ...
            # You can change this step for different coupling.

            #J0 = np.full((2**n, 2**n, 4), 1)  # Constant coupling
            #J0 = np.random.randn(2**n, 2**n, 4)/5  # Standard Gaussian coupling
            J0=2*np.random.randint(0,2,(2**n, 2**n, 4))-1# random \pm J coupling
            #J0=np.zeros((2**n, 2**n, 4))
            ...
            if(np.isnan(Q[i][j])or np.isnan(q1) or (np.isnan(q2))):
                j=j-1
                #If nan,then run again.
            j=j+1
    ...
    to_plot = True
    if(to_plot):
        x_ticks = np.arange(1, 5, 0.1)
        plt.plot(kT_array, F_average)
        plt.xlabel("kT")
        plt.ylabel("F")
        plt.xticks(x_ticks)
